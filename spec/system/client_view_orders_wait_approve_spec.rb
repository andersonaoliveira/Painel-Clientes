...
describe 'Cliente visualiza pedidos aguardando aprovação' do
  it 'a partir de um link no painel de clientes' do
    ...
    visit root_path
    click_on 'Pedidos'
    select 'Aguardando Aprovação', from: 'status'
    click_on 'Filtrar'
  ...
  it 'e não tem pedidos aguardando aprovação' do
    client = create(:client)
    order = create(:order, client: client)
    create(:product, order: order, client: client)
    allow(Order).to receive(:pull_orders).and_return(order)
  ...
  it ' e exibe pedidos aguardando aprovação' do
    client = create(:client)
